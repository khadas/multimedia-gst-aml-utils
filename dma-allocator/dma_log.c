#include <stdarg.h>
#include <stdio.h>
#include <stdlib.h>

#include "dma_log.h"

#include <fcntl.h>
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <string.h>
#include <unistd.h>
#include <sys/ioctl.h>
#include <sys/mman.h>

#define DEBUG_BUFFER_LEN 1280


dma_debug_level_t g_log_level = DMA_DEBUG_LEVEL_ERROR;

void dma_set_log_level(dma_debug_level_t level,dma_log_format_t output_format)
{
	(void) output_format;
    char* log_level = getenv("DMA_LOG_LEVEL");

    if (log_level) {
        g_log_level = (dma_debug_level_t)atoi(log_level);
        LOGW("Set log level by environmental variable.level=%d",g_log_level);
    } else {
        g_log_level = level;
        LOGW("Set log level=%d",g_log_level);
    }
    LOGW("output_format not support Imperfect, default to DMA_LOG_TERMINAL");

	return;
}

static int _check_log_level(dma_debug_level_t level)
{
    dma_debug_level_t env_log_level;
    char* log_level = getenv("DMA_LOG_LEVEL");

    if (log_level) {
        env_log_level = (dma_debug_level_t)atoi(log_level);
        if (env_log_level != g_log_level) {
            g_log_level = env_log_level;
            LOGW("Set log level by environmental variable..level=%d",g_log_level);
        }
    }

    if (g_log_level >= level) {
        return 1;
    }
    return 0;
}

void dma_LogMsg(dma_debug_level_t level, const char *fmt, ...)
{
    char arg_buffer[DEBUG_BUFFER_LEN] = {0};
    va_list arg;

    if (_check_log_level(level) == 0) {
        return ;
    }

    va_start(arg, fmt);
    vsnprintf(arg_buffer, DEBUG_BUFFER_LEN, fmt, arg);
    va_end(arg);

    fprintf(stderr, "%s\n", arg_buffer);
} /* dma_LogMsg() */
