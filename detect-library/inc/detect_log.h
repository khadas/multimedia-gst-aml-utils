...
#define LOGW( fmt, ... ) \
    detect_nn_LogMsg(DET_DEBUG_LEVEL_ERROR,  "W %s[%s:%d]" fmt, DETECT_API, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define LOGI( fmt, ... ) \
    detect_nn_LogMsg(DET_DEBUG_LEVEL_ERROR,  "I %s[%s:%d]" fmt, DETECT_API, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define LOGP( fmt, ... ) \
    detect_nn_LogMsg(DET_DEBUG_LEVEL_ERROR, "D %s[%s:%d]" fmt, DETECT_API, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define LOGD( fmt, ... ) \
    detect_nn_LogMsg(DET_DEBUG_LEVEL_ERROR, "D %s[%s:%d]" fmt, DETECT_API, __FUNCTION__, __LINE__, ##__VA_ARGS__)
#define __LOG__( fmt, ... ) \
    detect_nn_LogMsg(DET_DEBUG_LEVEL_ERROR, "%s[%s:%d]" fmt, DETECT_API, __FUNCTION__, __LINE__, ##__VA_ARGS__)
